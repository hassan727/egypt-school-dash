...
-- Seed Data for Stages and Classes
INSERT INTO stages (name) VALUES
('رياض الأطفال'),
('الابتدائية'),
('الإعدادية'),
...
-- Insert Classes (Example hierarchy)
-- You might want to customize this based on actual data
DO $$
DECLARE
    kg_id UUID;
    pri_id UUID;
    ...
    sec_id UUID;
BEGIN
    SELECT id INTO kg_id FROM stages WHERE name = 'رياض الأطفال';
    SELECT id INTO pri_id FROM stages WHERE name = 'الابتدائية';
    SELECT id INTO prep_id FROM stages WHERE name = 'الإعدادية';
    SELECT id INTO sec_id FROM stages WHERE name = 'الثانوية';

    -- KG Classes
    INSERT INTO classes (name, stage_id) VALUES
    ('KG1', kg_id), ('KG2', kg_id)
    ON CONFLICT (name, stage_id) DO NOTHING;

    -- Primary Classes
    INSERT INTO classes (name, stage_id) VALUES
    ('الصف الأول', pri_id), ('الصف الثاني', pri_id), ('الصف الثالث', pri_id),
    ('الصف الرابع', pri_id), ('الصف الخامس', pri_id), ('الصف السادس', pri_id)
    ON CONFLICT (name, stage_id) DO NOTHING;

    -- Prep Classes
    INSERT INTO classes (name, stage_id) VALUES
    ('الصف الأول', prep_id), ('الصف الثاني', prep_id), ('الصف الثالث', prep_id)
    ON CONFLICT (name, stage_id) DO NOTHING;

    -- Secondary Classes
    INSERT INTO classes (name, stage_id) VALUES
    ('الصف الأول', sec_id), ('الصف الثاني', sec_id), ('الصف الثالث', sec_id)
    ON CONFLICT (name, stage_id) DO NOTHING;
END $$;
