-- =============================================
-- Migration: FIX Missing Tables (Forced)
-- 20251215140000_fix_missing_tables.sql
-- =============================================
...
-- Safely add absence_policies if missing (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'hr_system_settings' AND column_name = 'absence_policies') THEN
...
CREATE TRIGGER update_hr_calendar_overrides_updated_at BEFORE UPDATE ON hr_calendar_overrides FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_attendance_locations_updated_at ON attendance_locations;
CREATE TRIGGER update_attendance_locations_updated_at BEFORE UPDATE ON attendance_locations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
