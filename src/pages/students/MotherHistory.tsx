...
const MotherHistory = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        // Get mother data audit trail
        const { data, error } = await supabase
          .from('student_audit_trail')
          .select('*')
          .eq('student_id', studentId)
          .eq('change_type', 'Mother Data')
          .order('created_at', { ascending: false });

        if (error) throw error;
        setAuditRecords(data || []);
      } catch (err) {
        console.error('Error fetching mother history:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const formatFieldName = (field: string) => {
    const fieldNames: Record<string, string> = {
      ...
      'mother_national_id': 'الرقم القومي',
      'mother_job': 'الوظيفة',
      'mother_workplace': 'جهة العمل',
      'mother_phone': 'رقم الهاتف',
      'mother_email': 'البريد الإلكتروني',
      'mother_education_level': 'المؤهل الدراسي',
      'mother_address': 'العنوان',
      'mother_relationship': 'صلة الأم بالطالب'
    };
      ...
      <div className="max-w-6xl mx-auto py-6 px-4">
        ...
        {!loading && auditRecords.length === 0 && (
          <Card className="p-8 text-center">
            ...
            <p className="text-gray-500">لم يتم إجراء أي تعديلات على بيانات الأم لهذا الطالب</p>
          </Card>
        )}
              ...
              <Card key={record.id} className="p-6 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">تعديل بيانات الأم</h3>
