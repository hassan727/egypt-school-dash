...
const EnrollmentHistory = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        // Get enrollment data audit trail
        const { data, error } = await supabase
          .from('student_audit_trail')
          .select('*')
          .eq('student_id', studentId)
          .eq('change_type', 'Enrollment Data')
  ...
  const formatFieldName = (field: string) => {
    const fieldNames: Record<string, string> = {
      ...
      'has_repeated': 'هل سبق الرسوب',
      'order_among_siblings': 'الترتيب بين الإخوة',
      'is_regular': 'حالة الطالب'
    };
      ...
      <div className="max-w-6xl mx-auto py-6 px-4">
        ...
        {!loading && auditRecords.length === 0 && (
          <Card className="p-8 text-center">
            ...
            <p className="text-gray-500">لم يتم إجراء أي تعديلات على بيانات القيد لهذا الطالب</p>
          </Card>
        )}
              ...
              <Card key={record.id} className="p-6 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">تعديل بيانات القيد</h3>
