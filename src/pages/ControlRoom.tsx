...
export default function ControlRoom() {
    ...
    // Auto-select first school if not selected
    useEffect(() => {
    ...
    // Fetch academic years when selected school changes
    useEffect(() => {
        const fetchYears = async () => {
            ...
            try {
                // Try fetching years for selected school
                ...
                if (years && years.length > 0) {
                    ...
                    if (!selectedYear) {
                        ...
                        setSelectedYear(activeYear?.year_code || years[0]?.year_code || '');
                    }
                } else {
                    // Fallback: fetch ALL years (unfiltered) for backward compat
                    ...
                    if (allYears && allYears.length > 0) {
                        ...
                        if (!selectedYear) {
                            const activeYear = allYears.find((y: any) => y.is_active);
                            setSelectedYear(activeYear?.year_code || allYears[0]?.year_code || '');
                        }
                    } else {
                        setAcademicYears([]);
                    }
