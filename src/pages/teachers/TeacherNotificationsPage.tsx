import { useParams, useNavigate } from 'react-router-dom';
import { DashboardLayout } from '@/components/DashboardLayout';
import { Skeleton } from '@/components/ui/skeleton';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { TeacherNavigation } from '@/components/TeacherProfile/TeacherNavigation';
import { NotificationsSection } from '@/components/TeacherProfile/NotificationsSection';
import { PrintOptionsModal } from '@/components/TeacherProfile/PrintOptionsModal';
import { useTeacherData } from '@/hooks/useTeacherData';
import { AlertTriangle, ArrowRight, Bell, MessageSquare } from 'lucide-react';
import { toast } from 'sonner';

export default function TeacherNotificationsPage() {
    const { teacherId } = useParams<{ teacherId: string }>();
    const navigate = useNavigate();
    const { teacherProfile, loading, error } = useTeacherData(teacherId || '');

    if (loading) {
        return (
            <DashboardLayout>
                <div className="space-y-6 max-w-7xl mx-auto py-6 px-4">
                    <Skeleton className="h-16 w-full" />
                    <Skeleton className="h-20 w-full" />
                    <Skeleton className="h-96 w-full" />
                </div>
            </DashboardLayout>
        );
    }

    if (error || !teacherProfile) {
        return (
            <DashboardLayout>
                <div className="text-center py-10 space-y-4">
                    <div className="p-6 bg-red-50 border border-red-200 rounded-lg max-w-md mx-auto">
                        <AlertTriangle className="h-12 w-12 text-red-500 mx-auto mb-4" />
                        <h2 className="text-xl font-semibold text-red-800 mb-2">خطأ في تحميل البيانات</h2>
                        <p className="text-red-600 mb-4">{error || 'المعلم غير موجود'}</p>
                        <Button
                            onClick={() => navigate('/teachers')}
                            variant="outline"
                            className="gap-2"
                        >
                            <ArrowRight className="h-4 w-4" />
                            قائمة المعلمين
                        </Button>
                    </div>
                </div>
            </DashboardLayout>
        );
    }

    const teacherName = teacherProfile.personalData.fullNameAr;
    const unreadCount = teacherProfile.notifications.filter(n => !n.isRead).length;

    // إرسال واتساب
    const handleWhatsApp = () => {
        const phone = (teacherProfile.personalData.whatsappNumber || teacherProfile.personalData.phone)?.replace(/\D/g, '');
        if (phone) {
            window.open(`https://wa.me/${phone}`, '_blank');
            toast.success('تم فتح واتساب');
        } else {
            toast.error('لا يوجد رقم واتساب مسجل');
        }
    };

    return (
        <DashboardLayout>
            <div className="space-y-6 max-w-7xl mx-auto py-6 px-4">
                {/* Navigation & Actions Bar */}
                <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                    <Button
                        variant="outline"
                        onClick={() => navigate('/teachers')}
                        className="gap-2 hover:bg-blue-50 hover:border-blue-300"
                    >
                        <ArrowRight className="h-4 w-4" />
                        العودة لقائمة المعلمين
                    </Button>

                    <div className="flex gap-2">
                        <Button
                            variant="outline"
                            onClick={handleWhatsApp}
                            className="gap-2 hover:bg-green-50 hover:border-green-300"
                        >
                            <MessageSquare className="h-4 w-4 text-green-600" />
                            واتساب
                        </Button>
                        <PrintOptionsModal teacherProfile={teacherProfile} />
                    </div>
                </div>

                {/* شريط التنقل */}
                <TeacherNavigation teacherId={teacherId || ''} activeSection="notifications" />

                {/* رأس الصفحة مع تدرج */}
                <div className="bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg p-6 shadow-lg">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="p-4 bg-white/20 rounded-xl">
                                <Bell className="h-8 w-8" />
                            </div>
                            <div>
                                <h1 className="text-3xl font-bold mb-1">الإشعارات</h1>
                                <p className="text-indigo-100">
                                    {teacherName} • الإشعارات والتنبيهات المرسلة
                                </p>
                            </div>
                        </div>
                        <div className="text-left flex items-center gap-3">
                            <div>
                                <p className="text-indigo-100 text-sm">غير مقروءة</p>
                                <p className="text-3xl font-bold">{unreadCount}</p>
                            </div>
                            {unreadCount > 0 && (
                                <span className="px-3 py-1 bg-red-500 text-white rounded-full text-sm font-medium animate-pulse">
                                    جديد
                                </span>
                            )}
                        </div>
                    </div>
                </div>

                {/* قسم الإشعارات */}
                <NotificationsSection notifications={teacherProfile.notifications} />
            </div>
        </DashboardLayout>
    );
}
