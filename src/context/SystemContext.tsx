...
export function SystemProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        const fetchSchools = async () => {
            ...
            try {
                const { data, error } = await supabase
                    .from('schools')
                    .select('id, school_name, school_code, settings, is_trial, trial_ends_at, status')
                    .order('school_name');
        ...
        // 🟢 Setup Real-time Subscription for 'schools' table
        const subscription = supabase
            ...
            .on(
                ...
                (payload) => {
                    console.log('🔄 Realtime update received for school:', payload.new.id);
                    // Update the specific school in the list
                    setSchools((prevSchools) =>
                        prevSchools.map((school) =>
                            school.id === payload.new.id ? { ...school, ...payload.new } : school
                        )
                    );

                    // If the updated school is the currently selected one, update it too
                    setSelectedSchool((prevSelected) => {
                        if (prevSelected && prevSelected.id === payload.new.id) {
                            const updated = { ...prevSelected, ...payload.new };
                            saveIdentity({ school: updated });
                            return updated;
                        }
                        return prevSelected;
                    });
    ...
    const setSchool = (school: School | null) => {
        ...
        saveIdentity({ school: school as School });
    };
