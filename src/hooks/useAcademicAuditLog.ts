...
export function useAcademicAuditLog() {
  ...
  const getAuditLogForStudent = async (
    ...
    try {
      ...
      return (data || []).map(item => ({
        ...
        gradeInfo: item.grades // معلومات التقييم المرتبط
      }));
    } catch (error) {
      console.error('خطأ في جلب سجل التدقيق:', error);
      return [];
  ...
  const searchAuditLog = async (
    ...
    try {
      ...
      if (filters.subject) {
        query = query.eq('grades.subject_name', filters.subject);
      }
      if (filters.assessmentType) {
        query = query.eq('grades.assessment_type', filters.assessmentType);
      }
